-- 색깔 순서 제보 — gym_submissions.changes.color_order 배열을 승인 시 반영.
--
-- changes JSONB 예:
--   { "color_order": ["yellow", "green", "blue", "red"] }
--
-- 처리: add_colors / remove_colors 적용된 다음에 실행 → 최종 색깔 상태에서 순서만 갱신.
-- 배열에 들어있는 색깔이 등록돼 있으면 그 배열 인덱스를 order_index 로 설정.
-- 배열에 없는 등록 색깔은 뒤로 밀어둔다 (현재 order_index 유지 + 큰 offset).
...
begin
  ...
  -- ─── 색깔 구성 변경 (add_colors / remove_colors / color_order) ──────────
  if new.status = 'approved' and old.status <> 'approved' then
    ...
    if v_target_gym is not null then
      ...
      -- color_order — 배열 인덱스를 그대로 order_index 로 설정.
      -- 배열에 없는 등록 색깔은 100 이상의 큰 offset 으로 밀어둠 (현 순서 유지).
      ...
      if v_arr is not null and jsonb_typeof(v_arr) = 'array' then
        ...
        -- 배열에 없는 row 들은 큰 값으로 밀어둠 (충돌 방지 + 뒤로 가도록)
        update gym_color_schemes
